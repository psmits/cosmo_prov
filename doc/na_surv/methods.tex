...
\begin{align}
  p(y_{i}|\alpha, \sigma) &= \mathrm{Weibull}(y_{i}|\alpha, \sigma_{i}) \nonumber \\ 
  &= \frac{\alpha}{\sigma_{i}} \left(\frac{y_{i}}{\sigma_{i}}\right)^{\alpha - 1} \exp\left(-\left(\frac{y_{i}}{\sigma_{i}}\right)^{\alpha}\right) 
  \label{eq:weibull}
\end{align}
...
To model censored data the survival function, \(S(t)\), must first be defined \citep{Klein2003}. For the Weibull distribution this is 
\begin{equation}
