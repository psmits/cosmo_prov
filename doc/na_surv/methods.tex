...
To allow for \(\sigma\) to vary with \(y\)'s covariate information it reparameterized in a similar fashion to \(\lambda\) with a few key differences. Because \(\sigma = 1/\lambda\) and to preserve interpretation of \(\beta\), along with taking into account the \(\alpha\), \(\sigma\) is reparameterized as
\begin{equation}
  \sigma_{i} = \exp\left(\frac{-(\beta^{T}\mathbf{X}_{i})}{\alpha}\right).
  \label{eq:reg}
\end{equation}
