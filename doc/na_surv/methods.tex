...
In total, there are three different variance components in this model: sample variance \(\sigma_{y}^{2}\), cohort variance \(\sigma_{c}^{2}\), and phylogenetic variance \(\sigma_{p}^{2}\). The model used here (Eq. \ref{eq:weibull}) does not include an estimate of overall variance. Instead, \(\sigma_{y}^{2}\) was approximated by a simulation procedure proposed by \citet{Goldstein2002}.

The procedure is as follows:
\begin{enumerate}
  \item Simulate \(w\) (50,000) values of \(\eta\) and \(h\); \(\eta \sim \mathcal{N}(0, \sigma_{c})\) and \(h \sim \mathcal{N}(0, \sigma_{p})\).
  \item For a given value of \(\beta^{T} \mathbf{X}\), calculate \(\sigma^{*}\) (Eq. \ref{eq:reg}) for all \(w\) simulations.
  \item Calculate \(\upsilon\), the Weibull variance (Eq. \ref{eq:wei_var}) of each element of \(\sigma^{*}\) with \(\alpha\) drawn from the posterior estimate.
  \item \(\sigma_{y}^{2} = \frac{1}{w} \sum_{i}^{w} \upsilon_{i}\).
\end{enumerate}
