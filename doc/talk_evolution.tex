...
\begin{frame}
  ...
  \begin{columns}
    \begin{column}{0.5\textwidth}
      % need an image about this. people don't talk about this, sadly.
    \end{column}
    \begin{column}{0.5\textwidth}
...
\begin{frame}
  \frametitle{Analysis and inference}

  \begin{columns}
    \begin{column}{0.5\textwidth}
      \begin{itemize}
        \item PBDB occurrences
        \item age \(\sim\) Exponential or Weibull
        \item \(\lambda \propto\) traits, \(k\) constant
        \item range in/out taxa \\right censored
        \item maxLik approach, \\multi-model inference
      \end{itemize}
    \end{column}
    \begin{column}{0.5\textwidth}
      % model selection
