...
% randomize for ``base line'' importance
\end{frame}

\begin{frame}
  \frametitle{Parameter interpretations}

  % NA: while weibull distribution is ``best'', k \approx 1, so exponential arguably safer
  %
  % Eur: both cases strongly favoring Weibull model over exponential
  
  % model averaged estimate of \(k\) (?)
