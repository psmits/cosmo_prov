...
namespace degree_full_model_model_namespace {
...
class degree_full_model_model : public prob_grad {
private:
    int N;
    int D;
    int M;
    ...
    matrix_d DS;
public:
    degree_full_model_model(stan::io::var_context& context__,
        std::ostream* pstream__ = 0)
        : prob_grad(0) {
        static const char* function__ = "degree_full_model_model_namespace::degree_full_model_model(%1%)";
        ...
        stan::math::validate_non_negative_index("zeroes", "N", N);
        zeroes = vector_d(N);
        ...
        for (int i = 1; i <= N; ++i) {
            stan::math::assign(get_base1_lhs(zeroes,i,"zeroes",1), 0);
        }
        stan::math::assign(vcv_inv, inverse(vcv));
        for (int i = 1; i <= N; ++i) {
            for (int j = 1; j <= N; ++j) {
                stan::math::assign(get_base1_lhs(DS,i,j,"DS",1), if_else(logical_eq(i,j),sum(row(adj,i)),0.0));
            }
        }
    ...
    void transform_inits(const stan::io::var_context& context__,
        ...
        if (!(context__.contains_r("sigma_spt")))
            throw std::runtime_error("variable sigma_spt missing");
        vals_r__ = context__.vals_r("sigma_spt");
        pos__ = 0U;
        context__.validate_dims("initialization", "sigma_spt", "double", context__.to_vec());
        double sigma_spt(0);
        sigma_spt = vals_r__[pos__++];
        try { writer__.scalar_lb_unconstrain(0,sigma_spt); } catch (const std::exception& e) {  throw std::runtime_error(std::string("Error transforming variable sigma_spt: ") + e.what()); }
        ...
        double p(0);
        p = vals_r__[pos__++];
        try { writer__.scalar_lub_unconstrain(0,1,p); } catch (const std::exception& e) {  throw std::runtime_error(std::string("Error transforming variable p: ") + e.what()); }

        if (!(context__.contains_r("spatial")))
            throw std::runtime_error("variable spatial missing");
        vals_r__ = context__.vals_r("spatial");
        pos__ = 0U;
        context__.validate_dims("initialization", "spatial", "vector_d", context__.to_vec(N));
        vector_d spatial(N);
        for (int j1__ = 0U; j1__ < N; ++j1__)
            spatial(j1__) = vals_r__[pos__++];
        try { writer__.vector_unconstrain(spatial); } catch (const std::exception& e) {  throw std::runtime_error(std::string("Error transforming variable spatial: ") + e.what()); }
    ...
    T__ log_prob(vector<T__>& params_r__,
        ...
        T__ sigma_spt;
        (void) sigma_spt;   // dummy to suppress unused var warning
        if (jacobian__)
            sigma_spt = in__.scalar_lb_constrain(0,lp__);
        else
            sigma_spt = in__.scalar_lb_constrain(0);
        ...
        Eigen::Matrix<T__,Eigen::Dynamic,1>  spatial;
        (void) spatial;   // dummy to suppress unused var warning
        if (jacobian__)
            spatial = in__.vector_constrain(N,lp__);
        else
            spatial = in__.vector_constrain(N);
        ...
        stan::math::assign(tau, (1 / sigma_spt));
        ...
        try { 
            check_greater_or_equal(function__,tau,0,"tau", (double *)0);
        } catch (const std::exception& e) { 
            throw std::domain_error(std::string("Invalid value of tau: ") + std::string(e.what()));
        };
        // model body
        {
            ...
            lp_accum__.add(cauchy_log<propto__>(sigma_spt, 0, 2.5));
            lp_accum__.add(uniform_log<propto__>(p, 0, 1));
            lp_accum__.add(multi_normal_prec_log<propto__>(spatial, zeroes, multiply((tau * tau),subtract(DS,multiply(p,adj)))));
            stan::math::assign(mu, add(add(add(add(add(add(beta_inter,multiply(beta_mass,mass)),multiply(diet,beta_diet)),multiply(move,beta_move)),phy),spatial),log(off)));
            lp_accum__.add(poisson_log_log<propto__>(degree, mu));
        }
    ...
    void get_param_names(std::vector<std::string>& names__) const {
        ...
        names__.push_back("sigma_spt");
        names__.push_back("p");
        names__.push_back("spatial");
    ...
    void write_array(RNG& base_rng__,
        ...
        static const char* function__ = "degree_full_model_model_namespace::write_array(%1%)";
        (void) function__; // dummy call to supress warning
        ...
        double sigma_spt = in__.scalar_lb_constrain(0);
        double p = in__.scalar_lub_constrain(0,1);
        vector_d spatial = in__.vector_constrain(N);
        ...
        vars__.push_back(sigma_spt);
        vars__.push_back(p);
        for (int k_0__ = 0; k_0__ < N; ++k_0__) {
            vars__.push_back(spatial[k_0__]);
        }
        ...
        stan::math::assign(mu, add(add(add(add(add(add(beta_inter,multiply(beta_mass,mass)),multiply(diet,beta_diet)),multiply(move,beta_move)),phy),spatial),log(off)));
        for (int i = 1; i <= N; ++i) {
            stan::math::assign(get_base1_lhs(log_lik,i,"log_lik",1), poisson_log_log(get_base1(degree,i,"degree",1),get_base1(mu,i,"mu",1)));
        }
    ...
    void write_csv(RNG& base_rng__,
        ...
        stan::io::reader<double> in__(params_r__,params_i__);
        stan::io::csv_writer writer__(o__);
        static const char* function__ = "degree_full_model_model_namespace::write_csv(%1%)";
        (void) function__; // dummy call to supress warning
        // read-transform, write parameters
        double beta_inter = in__.scalar_constrain();
        ...
        writer__.write(phy);
        double sigma_spt = in__.scalar_lb_constrain(0);
        writer__.write(sigma_spt);
        double p = in__.scalar_lub_constrain(0,1);
        writer__.write(p);
        vector_d spatial = in__.vector_constrain(N);
        writer__.write(spatial);
        double phi = in__.scalar_lb_constrain(0);
        writer__.write(phi);
    ...
    static std::string model_name() {
        return "degree_full_model_model";
    }
    ...
    void constrained_param_names(std::vector<std::string>& param_names__,
        ...
        for (int k_0__ = 1; k_0__ <= N; ++k_0__) {
            param_name_stream__.str(std::string());
            param_name_stream__ << "spatial" << '.' << k_0__;
            param_names__.push_back(param_name_stream__.str());
        }
        param_name_stream__.str(std::string());
...
typedef degree_full_model_model_namespace::degree_full_model_model stan_model;
